AddRemoteEvent("BuildDialog", function(player)
    ...
    for id, table in pairs(_Dialog_admin["section"]) do
        ...
        CallRemoteEvent(player, "BuildNav", name, cur_id, section_id)
    ...
    for section_id, tableSection in pairs(_Dialog_admin["section"]) do
        for type, data in ipairs(tableSection["form"]) do
            ...
            if type == "text" or type == "color" then
                CallRemoteEvent(player, "BuildInput", section_id, type, data['id'], data['custom'], data['name'])
            end
        
            if type == "player" or type == "vehicles" or type == "weapons" or type == "preset_pos" or type == "items" or type == "player_cache" then
                CallRemoteEvent(player, "BuildGameSelect", section_id, type, data['id'], data['custom'], data['name'])
            end
            ...
            if type == "select" then
                ...
                CallRemoteEvent(player, "BuildSelect", section_id, data['id'], data['custom'], data['name'], options)
            end

            if type == "checkbox" then
                CallRemoteEvent(player, "BuildCheckbox", section_id, data['name'], data['id'], data['checked'])
            end
        end
    end
